...
#在训练和测试阶段，使用该类读取用于训练和测试的图像和标签，同时也完成数据预处理。
class VOCDataset(data.Dataset):
    ...
    def pull_item(self, index):
        if random.random() < self.mosaic_prob: #生成0-1之间的随机数
            # load a mosaic image 马赛克增强
            mosaic = True
...
if __name__ == "__main__":
    ...
    # opt
    parser.add_argument('--root', default='F:\\subject\\Graduation_Project\\dataset\\VOCdevkit',
                        help='data root')
    parser.add_argument('-size', '--img_size', default=640, type=int,
                        help='input image size.')
    parser.add_argument('--aug_type', type=str, default='ssd', #数据增强
    ...
    dataset = VOCDataset(
        ...
        data_dir=args.root,
        image_sets=[('2012', 'train')] if args.is_train else [('2012', 'val')],
